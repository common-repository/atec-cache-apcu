...
if (isset($wp_object_cache->cache_hits))
{
	...
	<table class="atec-table atec-table-tiny atec-table-td-first">
		<tbody>
			<tr><td>', esc_attr__('Hits','atec-cache-apcu'), ':</td><td>', esc_attr(number_format($wp_object_cache->cache_hits).sprintf(" (%.1f%%)",$hits)), '</td></tr>
			<tr><td>', esc_attr__('Misses','atec-cache-apcu'), ':</td><td>', esc_attr(number_format($wp_object_cache->cache_misses).sprintf(" (%.1f%%)",$misses)), '</td></tr>
		</tbody>
	</table>';
...
if (defined('LSCWP_OBJECT_CACHE') && LSCWP_OBJECT_CACHE=='true' && (method_exists('WP_Object_Cache', 'debug'))) 
{
	...
	<table class="atec-table atec-table-tiny atec-table-td-first">
	<tbody>
		<tr><td>', esc_attr__('Items','atec-cache-apcu'), ':</td><td>', esc_attr(number_format($ls_total)), '</td></tr>
		<tr><td>', esc_attr__('Hits','atec-cache-apcu'), ':</td><td>', esc_attr(number_format($ls_hit).sprintf(" (%.1f%%)",$hits)), '</td></tr>
		<tr><td>', esc_attr__('Misses','atec-cache-apcu'), ':</td><td>', esc_attr(number_format($ls_miss).sprintf(" (%.1f%%)",$misses)), '</td></tr>
	</tbody>
	</table>';
